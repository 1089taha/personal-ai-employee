"""
Morning Briefing — Daily Briefing Trigger
Invoked by PM2 cron at 7:00 AM. Calls Claude Code in non-interactive mode with the
morning_briefing prompt. Runs once per execution — PM2 handles the schedule.
"""
...
prompt = """
Read the skill at .claude/skills/morning_briefing.md and execute it fully.
Collect all data from the vault, generate today's morning briefing,
save it to /Pending_Approval/, and update Dashboard.md.
"""
...
def main() -> None:
    log.info("[MorningBriefing] Generating daily briefing...")
    ...
    try:
        ...
        log.error("[MorningBriefing] ERROR — Claude Code not installed or not in PATH")
        sys.exit(1)
    except subprocess.TimeoutExpired:
        log.error("[MorningBriefing] ERROR — Claude Code timed out after 10 minutes")
        sys.exit(1)
    except Exception as exc:
        log.error("[MorningBriefing] ERROR — %s", exc)
        sys.exit(1)
    ...
    if result.returncode != 0:
        ...
        log.error(
            "[MorningBriefing] ERROR — Claude Code exited with code %d%s",
            result.returncode,
            f"\nstderr: {stderr}" if stderr else "",
        )
        sys.exit(result.returncode)

    log.info("[MorningBriefing] Done — briefing saved to Pending_Approval/")
