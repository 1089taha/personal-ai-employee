"""
Content Calendar — Weekly Planning Trigger
Invoked by PM2 cron every Sunday at 9:00 AM. Calls Claude Code in non-interactive
mode with the content_calendar prompt. Runs once per execution — PM2 handles the
schedule.
"""
...
prompt = """
Read the skill at .claude/skills/content_calendar.md and execute it fully.
Generate a 7-day LinkedIn content calendar for the coming week and save it
to /Pending_Approval/.
"""
...
def main() -> None:
    log.info("[ContentCalendar] Generating weekly content calendar...")
    ...
    try:
        ...
        log.error("[ContentCalendar] ERROR — Claude Code not installed or not in PATH")
        sys.exit(1)
    except subprocess.TimeoutExpired:
        log.error("[ContentCalendar] ERROR — Claude Code timed out after 10 minutes")
        sys.exit(1)
    except Exception as exc:
        log.error("[ContentCalendar] ERROR — %s", exc)
        sys.exit(1)
    ...
    if result.returncode != 0:
        ...
        log.error(
            "[ContentCalendar] ERROR — Claude Code exited with code %d%s",
            result.returncode,
            f"\nstderr: {stderr}" if stderr else "",
        )
        sys.exit(result.returncode)

    log.info("[ContentCalendar] Done — calendar saved to Pending_Approval/")
